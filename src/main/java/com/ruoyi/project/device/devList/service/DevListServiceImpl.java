...
//        }
     ...
     */
    @Override
    public int removeJsCodeById(Integer id) {
        DevList devList = devListMapper.selectDevListById(id);
        if (devList == null || devList.getDefId().equals(DevConstants.BE_DELETED)) {
            throw new BusinessException("硬件不存在或被删除");
        }
        if (devList.getSign() == DevConstants.DEV_SIGN_USED) {
            throw new BusinessException("被配置的硬件不能删除");
        }
        return devListMapper.removeJsCodeById(id);
    }
}
