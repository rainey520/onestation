...
public class WorkstationServiceImpl implements IWorkstationService 
{
	...
	public int appLineCnfStation(Workstation workstation) {
		...
		if (work.getcId() != 0) {
			...
			if (workstation.getcCode().equals(work.getcCode())) {
				...
				// 还原之前硬件配置
				devListMapper.updateDevSignAndType(work.getcId(),DevConstants.DEV_SIGN_NOT_USE,null,null);
				// 新增配置
				work.setcId(devList.getId());
				work.setcCode(devList.getDeviceId());
				devListMapper.updateDevSignAndType(devList.getId(),DevConstants.DEV_SIGN_USED,DevConstants.DEV_TYPE_LINE,(line.getLineName() + "--" +work.getwName()));
			}
		} else {
			// 新增配置
			work.setcId(devList.getId());
	...
	public int deleteWorkstationById(Integer id)
	{
		 ...
		 */
		if (work.getDevId() > 0) {
			devListMapper.updateDevSignAndType(work.getDevId(),DevConstants.DEV_SIGN_NOT_USE,null,null);
		}
		if (work.getcId() > 0) {
			devListMapper.updateDevSignAndType(work.getcId(),DevConstants.DEV_SIGN_NOT_USE,null,null);
		}
		if (work.geteId() > 0) {
		    devListMapper.updateDevSignAndType(work.geteId(),DevConstants.DEV_SIGN_NOT_USE,null,null);
		}
		return row;
	}
